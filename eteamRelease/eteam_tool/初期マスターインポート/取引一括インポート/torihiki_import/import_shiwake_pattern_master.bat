...
SET /P ANSWER="スキーマ(%SCHEMA_NAME%)に対して仕訳パターンマスター情報をインポートしてよろしいですか(Y/N)?"
if /i {%ANSWER%} neq {y} exit /b 1

rem テーブル変更
psql -U eteam -d eteam -f .\files\import_shiwake_pattern_master.sql -v SCHEMA_NAME=%SCHEMA_NAME%
