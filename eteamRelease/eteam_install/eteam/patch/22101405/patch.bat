...
if "%ZAIMUVER%" EQU "SIAS" (
    if exist .\files\csv\setting_info_sias.csv copy /Y .\files\csv\setting_info_sias.csv				.\files\csv\setting_info_tmp.csv
) else if "%ZAIMUVER%" EQU "de3" (
    if exist .\files\csv\setting_info_de3.csv copy /Y .\files\csv\setting_info_de3.csv				.\files\csv\setting_info_tmp.csv
) else (
    echo バージョン判別でエラー
    exit /b 1
)
