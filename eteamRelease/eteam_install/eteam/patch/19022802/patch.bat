...
rem rem SQL側読取用一時ファイルを作成 ◆◆◆会社設定の更新時のみ◆◆◆
rem if "%ZAIMUVER%" EQU "SIAS" (
rem     copy /Y .\files\csv\setting_info_sias.csv				.\files\csv\setting_info_tmp.csv
rem ) else if "%ZAIMUVER%" EQU "de3" (
rem     copy /Y .\files\csv\setting_info_de3.csv				.\files\csv\setting_info_tmp.csv
rem ) else (
