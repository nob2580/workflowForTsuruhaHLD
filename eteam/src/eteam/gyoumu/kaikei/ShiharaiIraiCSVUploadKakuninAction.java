...
public class ShiharaiIraiCSVUploadKakuninAction extends EteamAbstractAction {
...
//＜画面制御情報＞
	...
	/** 支払依頼入力方式 */
	String nyuryokuHoushiki = setting.zeiDefaultA009();
...
//＜イベント＞
	...
	public String init(){
		try(EteamConnection connection = EteamConnection.connect()) {
			...
			//セション情報を取得する。
			ShiharaiIraiCSVUploadSessionInfo sessionInfo = (ShiharaiIraiCSVUploadSessionInfo)session.get(Sessionkey.SHIHARAIIRAI_CSV);
			if (null == sessionInfo) {
	...
	public String touroku(){
		try(EteamConnection connection = EteamConnection.connect()) {
			...
			// 処理
			startThread(sessionInfo, EteamCommon.getContextSchemaName());
	...
	protected void startThread(ShiharaiIraiCSVUploadSessionInfo sessionInfo, String contextSchemaName) {
		ShiharaiIraiCSVUploadThread myThread = new ShiharaiIraiCSVUploadThread(session, sessionInfo, contextSchemaName);
		myThread.start();
	}
}
