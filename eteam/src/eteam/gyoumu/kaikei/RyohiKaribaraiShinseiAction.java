...
import eteam.database.dao.RyohiKaribaraiMeisaiDao;
...
public class RyohiKaribaraiShinseiAction extends EteamEkispertCommon {
...
//＜画面入力以外＞
	...
	/**  画面項目制御クラス(申請内容・旅費明細） */
	GamenKoumokuSeigyo ks = new GamenKoumokuSeigyo(DENPYOU_KBN.RYOHI_KARIBARAI_SHINSEI);
	...
	String dairiFlg = "0";
	/** 仮払選択制御 */
	String karibaraiSentakuFlg = setting.karibaraiSentakuSeigyo();
	/** 予算執行対象フラグ */
	boolean yosanShikkouTaishouFlg;
	/** セキュリティパターンで使用できる部門かどうか */
	boolean enableBumonSecurity;
	/** 差引項目表示フラグ */
	boolean sasihikiHyoujiFlg = (null != sashihikiName && !sashihikiName.isEmpty());
	...
	//タイトル
	/** A005:出張伺い申請（仮払申請）の仮払あり 伝票種別（タイトル） */
	String denpyouShubetsu;
	/** A005:出張伺い申請（仮払申請）の仮払なし 伝票種別（タイトル） */
...
//＜部品＞
	...
	/** 旅費仮払ロジック */
	RyohiKaribaraiShinseiLogic RyohiKaribaraiShinseiLogic;
	/** 部門ユーザー管理ロジック */
	...
	/** 旅費仮払明細DAO */
	RyohiKaribaraiMeisaiDao ryohiKaribaraiMeisaiDao;
	/** 内部コード設定Dao */
...
//＜入力チェック＞
	...
	protected void denpyouFormatCheck() {
		...
		for (int i = 0; i < shubetsuCd.length; i++) {
			...
			if ( (karibaraiOn != null && karibaraiOn.equals("1")) || shubetsuCd[i].equals(RYOHISEISAN_SYUBETSU.KOUTSUUHI) ) {
				...
				//単価は数量入力タイプが「1」又は「2」の時、小数点以下3桁の入力チェックで判断する。
	...
	protected void soukanCheck(String user_Id) {
		...
		if (isNotEmpty(shiwakeEdaNoRyohi)) {
			...
			shiwakeCheckDataKashi.projectNm = "貸方" +  ks1.project.getName() + "コード";
			shiwakeCheckDataKashi.segmentNm = "貸方" + ks1.segment.getName() + "コード";
		...
		for (int i = 0; i < zeiritsu.length; i++) {
			String ip = "その他 経費 " + (i + 1);
			if(isNotEmpty(shiwakeEdaNo[i]))commonLogic.checkZeiritsu(kazeiKbn[i], toDecimal(zeiritsu[i]), keigenZeiritsuKbn[i], toDate(shiyoubi[i]), errorList, ip + "行目：");
		}
		...
		if (!shiwakeEdaNo[0].isEmpty()) {
			...
			for (int i = 0; i < shiwakeEdaNo.length; i++) {
				...
				//各経費明細の仕訳パターン
				GMap shiwakePattern = kaikeiLogic.findShiwakePattern(DENPYOU_KBN.KEIHI_TATEKAE_SEISAN, Integer.parseInt(shiwakeEdaNo[i]));
				...
				commonLogic.checkShiwake(DENPYOU_KBN.KEIHI_TATEKAE_SEISAN, SHIWAKE_PATTERN_SETTING_KBN.KARIKATA, shiwakeCheckData, daihyouBumonCd, errorList);
				...
				case SHIHARAI_HOUHOU.FURIKOMI:
					...
					commonLogic.checkShiwake(DENPYOU_KBN.KEIHI_TATEKAE_SEISAN, SHIWAKE_PATTERN_SETTING_KBN.KASHIKATA1, shiwakeCheckDataKashi1, daihyouBumonCd, errorList);
					break;
				...
				case SHIHARAI_HOUHOU.GENKIN:
					...
					commonLogic.checkShiwake(DENPYOU_KBN.KEIHI_TATEKAE_SEISAN, SHIWAKE_PATTERN_SETTING_KBN.KASHIKATA2, shiwakeCheckDataKashi1, daihyouBumonCd, errorList);
					break;
				}
		...
		if( sasihikiHyoujiFlg ){
			...
			if (meisaiKingaku != null) for (String k : meisaiKingaku) {
				if (isNotEmpty(k)) hontaiGoukei += toDecimal(k).doubleValue();
			}
			if (isNotEmpty(sashihikiKingaku)) hontaiGoukei += toDecimal(sashihikiKingaku).doubleValue();
			if (hontaiGoukei < 0) errorList.add("交通費と日当・宿泊費等の合計がマイナスです。");
		}
	}
...
//＜伝票共通から呼ばれるイベント処理＞
	...
	protected void displayKobetsuDenpyou(EteamConnection connection) {
		...
		if ( (isEmpty(super.denpyouId) && isEmpty(super.sanshouDenpyouId)) || isNotEmpty(shiyoushaHenkouShainNo)) {
			...
			// 申請と取引が1:1で決まる場合(「仮払あり」のみのとき)
			List<ShiwakePatternMaster> list = this.shiwakePatternMasterDao.loadByDenpyouKbn(DENPYOU_KBN.RYOHI_KARIBARAI_SHINSEI, this.futanBumonCdRyohi);
			if(!list.isEmpty() && setting.karibaraiSentakuSeigyo().equals("1")) {
				if(list.size() == 1) {
		...
		//登録済伝票の表示状態作成
			...
			GMap shinseiData = kaikeiLogic.findRyohiKaribarai(denpyouId);
			shinseiData2Gamen(shinseiData, sanshou, connection);
			List<GMap> meisaiList = kaikeiLogic.loadRyohiKaribaraiMeisai(denpyouId);
			if (!meisaiList.isEmpty()) {
				meisaiData2Gamen(meisaiList);
			} else {
				makeDefaultMeisai();
			}
			List<GMap> meisaiKeiriList = kaikeiLogic.loadRyohiKeihiKaribaraiMeisai(denpyouId);
		...
		//参照起票の表示状態作成
			...
			sanshou = true;
			GMap shinseiData = kaikeiLogic.findRyohiKaribarai(sanshouDenpyouId);
			...
			shinseiData2Gamen(shinseiData, sanshou, connection);
			List<GMap> meisaiList = kaikeiLogic.loadRyohiKaribaraiMeisai(sanshouDenpyouId);
			if (!meisaiList.isEmpty()) {
				if(getWfSeigyo().isOthersSanshouKihyou()) {
			...
			List<GMap> meisaiKeiriList = kaikeiLogic.loadRyohiKeihiKaribaraiMeisai(sanshouDenpyouId);
			if (!meisaiKeiriList.isEmpty()) {
				meisaiData2GamenKeihi(meisaiKeiriList, sanshou, shinseiData, connection);
			} else {
				makeDefaultMeisaiKeihi();
	...
	protected void tourokuKobetsuDenpyou(EteamConnection connection) {
		...
		//申請内容登録
		RyohiKaribaraiShinseiLogic.insertShinsei(
			denpyouId,
			userIdRyohi,
			shainCd,
			userSei,
		...
		if (!shubetsuCd[0].isEmpty()) {
			for (int i = 0; i < shubetsuCd.length; i++) {
				RyohiKaribaraiMeisai dto = new RyohiKaribaraiMeisai();
				...
				ryohiKaribaraiMeisaiDao.insert(dto, super.getUser().getTourokuOrKoushinUserId());
			}
		}
		...
		if (!shiwakeEdaNo[0].isEmpty()) {
			for (int i = 0; i < shiwakeEdaNo.length; i++) {
				...
				RyohiKaribaraiShinseiLogic.insertKeihiMeisai(
					denpyouId,
					i + 1,
	...
	protected void koushinKobetsuDenpyou(EteamConnection connection) {
		...
		//申請内容更新
		RyohiKaribaraiShinseiLogic.updateShinsei(
			denpyouId,
			userIdRyohi,
			shainCd,
			userSei,
		...
		//明細削除＆登録
		ryohiKaribaraiMeisaiDao.delete(denpyouId);
		RyohiKaribaraiShinseiLogic.deleteKeihiMeisai(denpyouId);
	...
	protected void shouninCheckKobetsuDenpyou(EteamConnection connection) {
		...
		if (commonLogic.isKeiriOrLastShounin(denpyouId,loginUserInfo) && karibaraiOn.equals("1")) {
			...
			String shiharai = formatDate(kaikeiLogic.findRyohiKaribarai(denpyouId).get("shiharaibi"));
	...
	protected boolean isKaribaraiMishiyouSeisanZumi(EteamConnection connection) {
		...
		boolean ret = false;
		GMap seisanWithKaribarai = kaikeiLogic.findRyohiSeisanWithKaribarai(denpyouId);
		if(null != seisanWithKaribarai){
			String seisanDenpyouId = seisanWithKaribarai.get("denpyou_id").toString();
			GMap seisan = kaikeiLogic.findRyohiSeisan(seisanDenpyouId);
...
//＜内部処理＞
	...
	protected void initParts(EteamConnection connection) {
		...
		RyohiKaribaraiShinseiLogic = EteamContainer.getComponent(RyohiKaribaraiShinseiLogic.class, connection);
		...
		ryohiKaribaraiMeisaiDao = EteamContainer.getComponent(RyohiKaribaraiMeisaiDao.class, connection);
	...
	protected void displaySeigyo(EteamConnection connection) {
		...
		if (shiharaiBiMode == 1  && isEmpty(shiharaibi) && shiharaihouhou.equals(SHIHARAI_HOUHOU.FURIKOMI)) {
			Date shiharai = null;
			if (setting.furikomiRuleA005().equals("1")) {
				shiharai = commonLogic.decideFurikomiDateHi(toDate(super.getKihyouBi()));
			} else if (setting.furikomiRuleA005().equals("2")) {
			...
			if (shiharai != null) {
				...
				RyohiKaribaraiShinseiLogic.updateShiharaibi(denpyouId, shiharai, loginUserId);
			}
		}
		...
		if (!"0".equals(setting.shinseiKikanCheckNissuu())
			...
			shinseiChuuki = setting.shinseiKikanCheckMongon();
			shinseiChuuki = shinseiChuuki.replace("<DAY>", setting.shinseiKikanCheckNissuu());
		}
		...
		if(isNotEmpty(denpyouId)) {
			GMap data = kaikeiLogic.findRyohiKaribarai(denpyouId);
	...
	protected void reloadShiwakePattern(EteamConnection connection) {
		...
		if (!shiwakeEdaNo[0].isEmpty()) {
			for (int i = 0; i < length; i++) {
				...
				// 取引
				GMap shiwakeP = kaikeiLogic.findShiwakePattern(DENPYOU_KBN.KEIHI_TATEKAE_SEISAN, Integer.parseInt(shiwakeEdaNo[i]));
				...
				case SHIHARAI_HOUHOU.FURIKOMI:
					//振込
					kashiTorihikisakiCd[i] = "".equals(shiwakeP.get("kashi_torihikisaki_cd1")) ? "" : torihikisakiCd[i];//貸方1　取引先コード
				...
				case SHIHARAI_HOUHOU.GENKIN:
					//現金
					kashiTorihikisakiCd[i] = "".equals(shiwakeP.get("kashi_torihikisaki_cd2")) ? "" : torihikisakiCd[i];//貸方2　取引先コード
	...
	protected void reloadMaster(EteamConnection connection) {
		//画面表示項目
		kamokuNameRyohi = masterLogic.findKamokuNameStr(kamokuCdRyohi);
		kamokuEdabanNameRyohi = masterLogic.findKamokuEdabanName(kamokuCdRyohi, kamokuEdabanCdRyohi);
		futanBumonNameRyohi = enableBumonSecurity ? masterLogic.findFutanBumonName(futanBumonCdRyohi) : "";
	...
	public void makeExcel(EteamConnection connection, OutputStream out) {
		RyohikaribaraiShinseiXlsLogic ryohikaribaraiseisanxlsLogic = EteamContainer.getComponent(RyohikaribaraiShinseiXlsLogic.class, connection);
		ryohikaribaraiseisanxlsLogic.makeExcel(denpyouId, out);
	}
