...
/**
 * 支払依頼書申請一括登録処理のスレッドクラスです。
 */
public class ShiharaiIraiCSVUploadThread extends Thread {
...
//＜部品＞
	 ...
	 * @param schema      スキーマ名
	 */
	public ShiharaiIraiCSVUploadThread(Map<String, Object> session, ShiharaiIraiCSVUploadSessionInfo sessionInfo, String schema){
	...
	public void touroku(){
		try(EteamConnection connection = EteamConnection.connect()) {
			...
			//伝票１つずつつくる
			for (ShiharaiIraiCSVUploadDenpyouInfo denpyou : denpyouList) {
				ShiharaiIraiAction action = makeDenpyouAction(denpyou);
				if ("success".equals(action.touroku())) {
					denpyou.setDenpyouId(action.getDenpyouId());
					//↑トランザクション別
					shiharaiIraiDao.updateCSVUploadFlg(action.getDenpyouId(), denpyou.getHasseiShubetsu());
					connection.commit();
				} else {
					//Actionで発生した例外はそのままthrowされる。ここでいうnot successは例外で終わっていないエラー。
	...
	protected ShiharaiIraiAction makeDenpyouAction(ShiharaiIraiCSVUploadDenpyouInfo cmnInfo) {
		//支払依頼Actionをインスタンス化
		ShiharaiIraiAction action = new ShiharaiIraiAction();
...
//		}
	...
	protected void setConnection(EteamConnection connection) {
		...
		shiharaiIraiDao = EteamContainer.getComponent(ShiharaiIraiDao.class, connection);
