...
function eventBtn(eventName) {
	...
	switch (eventName) {
	case 'entry':
		formObject.action = 'ryohi_karibarai_shinsei_touroku';
		break;
	case 'update':
		formObject.action = 'ryohi_karibarai_shinsei_koushin';
		break;
	case 'shinsei':
		formObject.action = 'ryohi_karibarai_shinsei_shinsei';
		break;
	case 'shounin':
		formObject.action = 'ryohi_karibarai_shinsei_shounin';
		break;
	case 'shouninkaijo':
		formObject.action = 'ryohi_karibarai_shinsei_shouninkaijo';
		break;
	}
 ...
 function changeShiyousha(dialogRetuserId, dialogRetusername, dialogRetshainNo, title, shiyoushaStr){
	...
	if(shainNoBefChg != "" && dialogRetshainNo.val() != shainNoBefChg){
		if(window.confirm(shiyoushaStr  + "の変更にあたり、入力内容がクリアされます。よろしいですか？")){
			...
			var hrefStr = "ryohi_karibarai_shinsei?denpyouKbn=A005&dairiFlg=1&shiyoushaHenkouShainNo=" + dialogRetshainNo.val();
