...
$.extend(Calculator.prototype, {
	...
	_finished: function(inst, label, value) {
		if (inst._inline) {
			...
			//コード改変：12桁以上の値が結果表示されていた場合は部品に反映しない
			if(value.indexOf(inst.options.decimalChar) > 12 || (value.indexOf(inst.options.decimalChar) == -1 && value.length > 12) ){
				return;
			}
