 ...
 * 旅費仮払抽出バッチ
 */
public class RyohiKaribaraiChuushutsuBat extends EteamAbstractKaikeiBat {

	/** ロガー */
	protected static EteamLogger log = EteamLogger.getLogger(RyohiKaribaraiChuushutsuBat.class);

	/** 抽出バッチのロジック */
	protected RyohiKaribaraiChuushutsuLogic gyoumuLogic;
	/** バッチ会計共通ロジック */
	...
	public int mainProc() {
		try {
			...
			log.info("出張伺い申請（仮払申請）抽出データ件数：" + denpyouList.size());
	...
	protected List<ShiwakeData> denpyouChuushutsu(GMap denpyou) {
		...
		if (kingaku != null && kingaku.doubleValue() > 0) {
			...
			sc.setTKY(common.cutTekiyou(common.shiwakeTekiyou(DENPYOU_KBN.RYOHI_KARIBARAI_SHINSEI, denpyou, null, "0")));
			sc.setTNO((String)denpyou.get("tekiyou_cd"));
			sc.setVALU((BigDecimal)denpyou.get("karibarai_kingaku"));
			shiwakes.add(shiwake);

			//複合の場合分割
			if(!EteamSettingInfo.getSettingInfo(Key.OP21MPARAM_DENPYOU_KEISHIKI_A005).equals("0")){
	...
	protected String getIdLevelRenkeiKey() {
		return EteamSettingInfo.Key.DENPYOU_SAKUSEI_TANI_A005;
	}
	...
	protected void initialize() {
		...
		gyoumuLogic = EteamContainer.getComponent(RyohiKaribaraiChuushutsuLogic.class, connection);
