 ...
 * 旅費仮払申請Logic
 */
public class RyohikaribaraiShinseiXlsLogic extends EteamAbstractLogic {
	/**
	...
	public void makeExcel(String denpyouId, OutputStream out) {
		...
		//テンプレートEXCEL開く
		// ---------------------------------------
		EteamXls excel = EteamXls.createBook(this.getClass().getResourceAsStream("template_ryohikaribaraishinsei.xls"), out);
		...
		// データ取得
		// ---------------------------------------
		GMap hontai = kaikeiLogic.findRyohiKaribarai(denpyouId); //ryohikaribarai
		List<GMap> 	meisaiList = kaikeiLogic.loadRyohiKaribaraiMeisai(denpyouId); //ryohikaribarai_meisai
		List<GMap> 	meisaiKeihiList = kaikeiLogic.loadRyohiKeihiKaribaraiMeisai(denpyouId); //ryohikaribarai_keihi_meisai
		String hosoku = (String)hontai.get("hosoku");
		GMap denpyouShubetuMap = workflowLogic.selectDenpyouShubetu(DENPYOU_KBN.RYOHI_KARIBARAI_SHINSEI);
		String shiharaihouhouCd = (String)hontai.get("shiharaihouhou");
		String shiwakeTekiyouNoCut = batchkaikeilogic.shiwakeTekiyou(DENPYOU_KBN.RYOHI_KARIBARAI_SHINSEI, hontai, null, "0");
		String version = "";
		int tekiyouByte = 0;
		...
		kaikeiXlsLogic.makeExcelMeisaiKeihi(meisaiKeihiList, excel, hontai, meisaiRow, meisaiCol);
		
		//■明細（日当・宿泊費等）
		kaikeiXlsLogic.makeExcelShoukei(RYOHISEISAN_SYUBETSU.SONOTA, meisaiList, excel, hontai, true, false);
		...
		kaikeiXlsLogic.makeExcelKokunaiMeisai(RYOHISEISAN_SYUBETSU.SONOTA, meisaiList, excel, meisaiRow02, true);
		
		//■明細（交通費）
		kaikeiXlsLogic.makeExcelShoukei(RYOHISEISAN_SYUBETSU.KOUTSUUHI, meisaiList, excel, hontai, true, false);
		countMeisaiShubetsu = kaikeiXlsLogic.courntMeisaiShubetsuCd(RYOHISEISAN_SYUBETSU.KOUTSUUHI, meisaiList);
		if (1 < countMeisaiShubetsu) excel.copyRow(meisaiRow01, meisaiRow01 + 1, 1, countMeisaiShubetsu - 1, true); // 一旦消す
		kaikeiXlsLogic.makeExcelKokunaiMeisai(RYOHISEISAN_SYUBETSU.KOUTSUUHI, meisaiList, excel, meisaiRow01, true);
