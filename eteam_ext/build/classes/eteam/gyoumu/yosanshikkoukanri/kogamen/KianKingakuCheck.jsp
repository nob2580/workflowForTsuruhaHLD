...
<div id='dialogMain'>
	...
	<section>
		...
		<div class='control-group'>
			...
			<label class='label'>起案番号</label>
			<span id='kianBangouColumn'>${su:htmlEscape(kianBangou)}</span>
		</div>
...
<c:if test='${"1" eq checkTani}'>
					...
					<th><nobr>起案額</nobr></th>
					<th><nobr>支出依頼済み額</nobr></th>
					<th><nobr>申請額</nobr></th>
					<th><nobr>起案残高</nobr></th>
...
<c:forEach var="record" items="${dataList}">
	<c:if test='${"" ne record.syuukeiBumonCd && "" eq record.meisaiBumonCd}'>
		...
		<c:if test='${"1" eq checkTani}'>
					<td class="text-l">${su:htmlEscape(record.kamokuName)}</td>
					<td class="text-l">${su:htmlEscape(record.kamokuEdabanName)}</td>
		</c:if>
					<td class="text-l"></td>
					...
					<td class='zandaka text-r'<c:if test='${fn:startsWith(su:htmlEscape(record.zandaka), "-")}'> style='color:red;'</c:if>>${su:htmlEscape(record.zandaka)}</td>
	...
	<c:if test='${"" eq record.syuukeiBumonCd}'>
		...
		<div>
			<p>金額チェックコメント</p>
		<textarea class='input-block-level' id='comment' placeholder='起案金額を超過している場合はコメントを入力してください。(200字)'>${su:htmlEscape(comment)}</textarea>
		</div>
	</section>
</div><!-- main -->
