...
<html lang="ja">
	<head>
		<meta charset="utf-8">
		<title>部門別取引一括登録｜<%=EteamSymbol.SYSTEM_NAME%></title>
 ...
 */
 function buttonAction(btnName) {
	...
	if(btnName == "upload"){
		...
		formObject.attr("action","bumonbetsu_torihiki_ikkatsu_touroku_upload");
		formObject.submit();
	}
	// ダウンロード
	if(btnName == "download"){
		formObject = $("form#uploadForm");
		formObject.attr("action","bumonbetsu_torihiki_ikkatsu_download");
		formObject.submit();
	}
	// 全件削除
	if(btnName == "delete"){
		if(window.confirm('全部門別取引設定を削除してもよろしいですか？')) {
			if(window.confirm('削除したデータは元に戻せません。本当によろしいですか？')) {
				formObject = $("form#uploadForm");
				formObject.attr("action","bumonbetsu_torihiki_ikkatsu_sakujo");
				formObject.submit();
			}
		}
	}
