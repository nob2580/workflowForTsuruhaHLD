...
<!-- カスタマイズここから -->
<c:if test='${0 < fn:length(sessionScope.user.kaishaKirikaeCd)}'>
				<p class='navbar-text pull-right'>会社切替
					<select name=kaishaKirikae class='input-large' onChange='commonKaishaKirikaeChange(this, "${su:htmlEscape(schemaName)}", "${su:htmlEscape(sessionScope.user.loginUserId)}", "${su:htmlEscape(sessionScope.user.gyoumuRoleId)}")'>
	<c:forEach var="hoge" items="${sessionScope.user.kaishaKirikaeCd}" varStatus="stat">
						<option value='${su:htmlEscape(sessionScope.user.kaishaKirikaeCd[stat.index])}' <c:if test="${sessionScope.user.kaishaKirikaeCd[stat.index] eq sessionScope.schemaName}">selected</c:if>>${su:htmlEscape(sessionScope.user.kaishaKirikaeName[stat.index])}</option>
	</c:forEach>
					</select>
					&nbsp;&nbsp;&nbsp;
				</p>
