...
<c:if test="${wfSeigyo.fileTenpuRef}">
	...
	<c:if test="${denpyouJoutai != '00'}">
			...
			<c:forEach var="record" items="${tempFileEbunshoList}">
				...
				<c:if test="${(ebunshoShiyouFlg eq '1' || ebunshoShiyouFlg eq '2') && ((record.ebunsho_no != null && record.ebunsho_no != '' ) || fn:containsIgnoreCase(su:htmlEscape(record.file_name),'.bmp') || fn:containsIgnoreCase(su:htmlEscape(record.file_name),'.jpeg') || fn:containsIgnoreCase(su:htmlEscape(record.file_name),'.jpg') || fn:containsIgnoreCase(su:htmlEscape(record.file_name),'.pdf'))}">
					...
					<c:if test="${ (record.touroku_time == null || record.touroku_time == '') && (record.ebunsho_no != null && record.ebunsho_no != '')}">
						...
						<c:if test="${ record.ebunsho_no != null && record.ebunsho_no != '' }">
									<br>
									<a href="ebunsho_file_download?denpyouId=${su:htmlEscape(denpyouId)}&denpyouKbn=${su:htmlEscape(denpyouKbn)}&downloadEbunshoNo=${su:htmlEscape(record.ebunsho_no)}" class="eblink">(${su:htmlEscape(record.ebunsho_no)}.pdf)</a>
						</c:if>
...
<!-- 									<label class='label ebKingaku'><c:if test='${record2.ebunsho_shubetsu ne "3" && kingaku_nini_flg[ebunsho_shubetsu_int] == 0}'><span class='required'>*</span></c:if>金額</label> -->
...
<!-- 2022/03/16 Ver22.03.09.01 登録後の品名枠を新規登録時と同じにする -->
...
function previewFile(current, prevOrNext, maxIndex)
{
	...
	var newLink = "../../static/tmp/" + newTenpuFileName + "." + newTenpuFileExtension;
	// PDF.jsの採用により、PDFだけリンク形式を変更
	var newPDFLink = "../../static/js/pdfjs-dist/web/viewer.html?file=../../../tmp/" + newTenpuFileName + "." + newTenpuFileExtension + "#pagemode=none";
