	...
	<c:if test='${status eq "End"}'>
		...
		<c:if test='${fn:length(errorList) ne 0}'>
			...
			<c:if test='${shozokuBumonWariate.errorFlg eq false}'>
					...
					<section>
						<h2>CSVファイル（会社切替）</h2>
						<b>${su:htmlEscape(csvFileNameKaishaKirikae)}</b>
					</section>
...
<c:if test='${status eq "End"}'>
								...
								<tbody>
<c:forEach var="kaishaKirikae" items="${kaishaKirikaeList}" varStatus="i">
									<tr>
										<td><nobr>${kaishaKirikae.number}</nobr></td>
	<c:if test='${status eq "End"}'>
		<c:if test='${fn:length(errorList) eq 0}'>
										<td><nobr>登録成功</nobr></td>
		</c:if>
		<c:if test='${fn:length(errorList) ne 0}'>
			<c:if test='${kaishaKirikae.errorFlg eq true}'>
										<td><nobr>登録失敗</nobr></td>
			</c:if>
			<c:if test='${kaishaKirikae.errorFlg eq false}'>
										<td><nobr>未登録</nobr></td>
			</c:if>
		</c:if>
				 
	</c:if>
										<td><nobr>${su:htmlEscape(kaishaKirikae.userId)}</nobr></td>
										<td><nobr>${su:htmlEscape(kaishaKirikae.schemeCd)}</nobr></td>
										<td><nobr>${su:htmlEscape(kaishaKirikae.yuukouKigenFrom)}</nobr></td>
										<td><nobr>${su:htmlEscape(kaishaKirikae.yuukouKigenTo)}</nobr></td>
										<td><nobr>${su:htmlEscape(kaishaKirikae.hyoujiJun)}</nobr></td>
									</tr>
</c:forEach>
								</tbody>
							</table>
