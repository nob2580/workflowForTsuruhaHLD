	...
	<c:forEach var="hoge" items="${gyoumuRoleId}" varStatus="stat">
		...
		<c:choose>
			...
			<c:otherwise>
					...
					<!-- カスタマイズここから -->
					<!-- 会社切替設定 -->
					<div id='kaishaKirikaeSetteiList' <c:if test="${!useKaishaKirikaeSettei}">style='display:none;'</c:if>>
						<section>
							<!-- サブタイトル -->
							<h2>会社切替設定</h2>
							<b><span></span></b>
							<div class='no-more-tables'>
								<div class='control-group'>
									<button type='button' onClick="kaishaKirikaeAdd()" class='btn'><i class='icon-plus'></i> 追加</button>
								</div>
								<table class='table-bordered table-condensed' style="margin-top:8px">
								<colgroup width="300">
									<thead>
									...
									<tbody id='kaishaKirikaeList'>
<c:forEach var="hoge" items="${kaishaKirikaeCd}" varStatus="stat">
										<tr style="vertical-align:top;">
											<td>
												${su:htmlEscape(kaishaKirikaeName[stat.index])}
												<input type="hidden" name='kaishaKirikaeCd'   value='${su:htmlEscape(kaishaKirikaeCd[stat.index])}'>
												<input type='hidden' name='kaishaKirikaeName' value='${su:htmlEscape(kaishaKirikaeName[stat.index])}'>
											</td>
											<td>
												<b><span class='required'>* </span></b>
												<input type='text' name='kaishaKirikaeYuukoukigenFrom' class='input-small datepicker' value='${su:htmlEscape(kaishaKirikaeYuukoukigenFrom[stat.index])}'>
												<span> ～ </span><br>&nbsp;&nbsp;
												<input type='text' name='kaishaKirikaeYuukoukigenTo' class='input-small datepicker' value='${su:htmlEscape(kaishaKirikaeYuukoukigenTo[stat.index])}'>
											</td>
											<td >
												<button type='button' name='kaishaKirikaeDel' class='btn btn-small'> 削除</button><br>
												<button type='button' name='kaishaKirikaeUp' class='btn btn-mini'>↑</button>
												<button type='button' name='kaishaKirikaeDown' class='btn btn-mini'>↓</button>
											</td>
										</tr>
</c:forEach>
									</tbody>
...
/**### 以下、会社切替の処理 ##############################
 *
 * （会社切替）追加ボタン押下時Function
 */
function kaishaKirikaeAdd() {

	dialogRetKaishaKirikaeSentakuCallback = function(bumonData){

		// 会社のセルを作成しデータを追加
		var kaishaKirikaeCd   = bumonData.attr("data-cd");
		var kaishaKirikaeName = bumonData.attr("data-name");

		//（会社切替）追加ボタン押下時アクション
		kaishaKirikaeBody = document.getElementById("kaishaKirikaeList");
		// 会社切替設定のBodyへ行追加
		kaishaKirikaeNewTR = kaishaKirikaeBody.insertRow(-1);

		//表の行の縦位置を指定
		$(kaishaKirikaeNewTR).css("vertical-align","top");

		// 会社切替名のセルを作成しデータを追加
		var input_bcd = document.createElement('input');
		input_bcd.type = 'hidden';
		input_bcd.name = 'kaishaKirikaeCd';
		input_bcd.value = kaishaKirikaeCd;

		var input_bnm = document.createElement('input');
		input_bnm.type = 'hidden';
		input_bnm.name = 'kaishaKirikaeName';
		input_bnm.value = kaishaKirikaeName;

		var elem = document.createElement('div');
		elem.appendChild(document.createTextNode(kaishaKirikaeName));
		elem.appendChild(input_bcd);
		elem.appendChild(input_bnm);

		newTD1 = kaishaKirikaeNewTR.insertCell(0).innerHTML = elem.innerHTML;

		// 有効期限のセルを作成しデータを追加
		newTD4 = kaishaKirikaeNewTR.insertCell(1).innerHTML = "<b><span class='required'>* <\/span><\/b><input type='text' name='kaishaKirikaeYuukoukigenFrom' class='input-small datepicker'><span> ～ <\/span><input type='text' name='kaishaKirikaeYuukoukigenTo' class='input-small datepicker'>";

		// 削除ボタンのセルを作成しボタンを作成
		newTD5 = kaishaKirikaeNewTR.insertCell(2).innerHTML = "<button type='button' name='kaishaKirikaeDel' class='btn btn-small'><\/i> 削除<\/button> <button type='button' name='kaishaKirikaeUp' class='btn btn-mini'>↑</button> <button type='button' name='kaishaKirikaeDown' class='btn btn-mini'>↓</button>";

		//追加した会社切替行へのアクション追加
		kaishaKirikaeActionAdd($("#kaishaKirikaeList").find("tr:last"));
	};
	commonKaishaKirikaeSentaku();
}

/**
 * （会社切替）削除ボタン押下時Function
 */
function kaishaKirikaeDel() {
	$(this).closest("tr").remove();
}
 ...
 * @param target
 */
function kaishaKirikaeActionAdd(target){
	// datepickerの初期化
	commonInit(target);

	//（会社切替）削除ボタン押下時アクション
	target.find("button[name=kaishaKirikaeDel]").click(kaishaKirikaeDel);

	//（会社切替）↑ボタン押下時アクション
	target.find("button[name=kaishaKirikaeUp]").click(kaishaKirikaeUp);

	//（会社切替）↓ボタン押下時アクション
	target.find("button[name=kaishaKirikaeDown]").click(kaishaKirikaeDown);
}
